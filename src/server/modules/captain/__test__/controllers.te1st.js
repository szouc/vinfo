...
describe('Captain Base Operations', () => {
  ...
  beforeAll(async () => {
    ...
    const fuelValues = [{ ...data.fuels[1], applicant: { username: data.drivers[2].username, fullname: data.drivers[2].fullname } }, { ...data.fuels[2], applicant: { username: data.drivers[2].username, fullname: data.drivers[2].fullname } }]
    const resFuel = await agent
      .post(`/api/vehicle/${vehicle0Id}/fuel`)
      .send(fuelValues)
    fuel0Id = resFuel.body.fuels[0]._id
    const maintainValues = [{ ...data.maintains[1], applicant: { username: data.drivers[2].username, fullname: data.drivers[2].fullname } }, { ...data.maintains[2], applicant: { username: data.drivers[2].username, fullname: data.drivers[2].fullname } }]
    ...
    transport1 = { assigner: { username: data.captains[0].username, fullname: data.captains[0].fullname }, vehicle: { _id: vehicle1Id }, from: { company: { _id: company0Id } }, to: { company: { _id: company1Id } }, product: { _id: product0Id } }
  })
  ...
  test('Should update a fuel by id', async () => {
    expect.assertions(2)
    const mapObj = { ':username': data.captains[0].username, ':childId': fuel0Id }
    const res = await agent
  ...
  test('Should update a maintain by id', async () => {
    expect.assertions(2)
    const mapObj = { ':username': data.captains[0].username, ':childId': maintain0Id }
    const res = await agent
  ...
  test('Should get all transports', async () => {
    ...
    await agent
      ...
      .send(transport1)
    const res = await agent.get(CAPTAIN_TRANSPORT_API.replace(/:username/, data.captains[0].username))
    expect(res.statusCode).toBe(200)
    expect(res.body).toHaveLength(2)
    expect(res.body[0]._id).toBe(transport0Id)
  })
  ...
  test('Should update a transport status', async () => {
    ...
    const res = await agent.put(replaceAll(CAPTAIN_TRANSPORT_ID_API, mapObj)).send({captain_status: 'pass'})
    expect(res.statusCode).toBe(200)
    expect(res.body.captain_status).toEqual('pass')
  })
})
